...
Физические полётные команды **заблокированы по умолчанию**. Для них одновременно нужны:

- административный запрос `DRONE_ENABLE_FLIGHT_TOOLS=1`;
- независимое разрешение от внешнего safety-контроллера на ROS-топике `DRONE_SAFETY_APPROVAL_TOPIC` (`std_msgs/Bool`).

Этот проект **не публикует разрешение сам**. Интеграция разрешения должна выполняться организатором соревнований или ответственным взрослым оператором с ручным перехватом. `drone_land` и `drone_hold_position` остаются доступными как защитные действия.
