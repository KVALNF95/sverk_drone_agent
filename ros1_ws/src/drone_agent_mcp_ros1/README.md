...
`drone_navigate_to_chess_cell` теперь выполняет многоэтапный профиль посадки: грубый подлёт к клетке, стабилизация, повторное выравнивание, снижение до малой высоты, повторное выравнивание и затем `land()`. Параметры профиля задаются через `CHESS_ALIGNMENT_*` и `CHESS_FINAL_APPROACH_ALTITUDE_M`.
